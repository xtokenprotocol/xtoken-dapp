...
class ContractWriter {
  ...
  public async nonce(address?: string): Promise<number> {
    return this.xtoken
      ...
      .eth.getTransactionCount(
        address || this.xtoken.store().getState().walletProvider.address,
      );
  }
  ...
  public async checkAndApproveContract(
    ...
    const address = this.xtoken.store().getState().walletProvider?.address;
    const dispatch = this.xtoken.store().dispatch;
  ...
  public async send(
    ...
    return new Promise<string | RevertInstructionError>((resolve, reject) => {
      return this.xtoken.writeContracts[contractName].methods[methodName](
        ...args,
      )
        .send(options)
