...
function calculateProfits(events: CustomEvent[]): CalculatedEvent | null {
  ...
  const prettyPrice = amount => {
    return events[0].loanToken === Asset.BNB
      ? amount
      : toWei(
          bignumber(1)
            .div(amount)
  ...
  if (events[0].collateralToken === Asset.BUSD) {
...
export function TradingHistory() {
  ...
  const tradeStates = useGetContractPastEvents('xtokenProtocol', 'Trade');
...
function HistoryTable(props: { items: CalculatedEvent[] }) {
  ...
  return (
    <div className="bg-white p-3 xtoken-border">
      <table {...getTableProps()} className="xtoken-table">
        <thead>
